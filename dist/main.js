...
function run() {
  return __async(this, null, function* () {
    ...
    if (!accessToken || accessToken === "") {
      core.setFailed("Parameter 'accessToken' is required to load all actions from the organization or user account");
      return;
    }
    if (organization === "") {
      core.setFailed("Parameter 'organization' is required to load all runners from it. Please provide one of them.");
      return;
    }
    ...
    if (repo !== "") {
      try {
        ...
        console.log(error);
        core.setFailed(`Could not authenticate with access token. Please check that it is correct and that it has the correct scope (see readme) on the repository: ${error}`);
        return;
      }
    }
